...
#define link_debug(x, ...) do { if ( link_debug_level >= x ) { if ( link_debug_level > 1 ) printf("%s():", __func__ ); printf(__VA_ARGS__); } } while(0)
#define link_error(...) do { if ( link_debug_level >= 1 ) { if ( link_debug_level > 1 ) printf("%s():", __func__ ); printf(__VA_ARGS__); } } while(0)
